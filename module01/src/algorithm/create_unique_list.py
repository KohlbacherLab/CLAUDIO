...
def create_list_of_unique_proteins(data, search_tool, blast_bin, blast_db, hhsearch_bin, hhsearch_db, verbose_level):
    # create pandas dataframe of unique proteins depending on uniprot ids
    #
    # input data: pd.DataFrame, search_tool: str, blast_bin: str/None, blast_db: str,
...
def search_pdb_entries(proteins, sequences, search_tool, blast_bin, blast_db, hhsearch_bin, hhsearch_db, verbose_level):
    ...
    for i, protein in enumerate(proteins):
        ...
        if search_tool == "blastp":
            ...
            cmd = f"{hhearch_call} -i {temp_path}tmp{i}.fasta -d {hhsearch_db}pdb70 -e 1e-5 " \
                  f"-blasttab {temp_path}tmp{i}.hhr -qid 90 -cov 50 -v 0 -cpu 20"
            os.system(cmd)
            # Open hhsearch output (Note: hhsearch outs cannot be retrieved from the commandline, as it is the case with
            # blastp)
            res = [line.split('\t')[1] for line in open(f"{temp_path}tmp{i}.hhr", 'r').read().split('\n')
