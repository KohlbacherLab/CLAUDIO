...
def main(input_filepath):
    ...
    print(f"number of unique proteins: {len(data[data.res_criteria_fulfilled]['unip_id_a'].unique())}")
    ...
    print(f"\n\nSuspicious datapoints:\n\talphafold + res_crit_false (total number = {len(data[(data.method_a == 'alphafold') & ~data.res_criteria_fulfilled].index)}):\n\t\t"
          f"{data[(data.method_a == 'alphafold') & ~data.res_criteria_fulfilled][['unip_id_a', 'pdb_id', 'chain', 'method_a', 'pub', 'res_criteria_fulfilled', 'eucl_dist', 'XL_type', 'final_XL_type']]}\n\t(publication + res_criteria_true)/publication:\n\t\t"
          ...
          f"\n\tres_criteria unfullfilled (total number = {len(data[~data.res_criteria_fulfilled].index)}): {data[~data.res_criteria_fulfilled][['unip_id_a', 'pos_a', 'pos_b', 'res_crit_a', 'res_crit_b']]}"
    ...
    print(f"\n\nFinal results:"
          ...
          f"({(sum(final_data.final_XL_type == 'inter') / total_dp) * 100:.2f}%)"
