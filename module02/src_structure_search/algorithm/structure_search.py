...
def structure_search(data, filename, search_tool, e_value, query_id, coverage, temp_path, blast_bin, blast_db,
    ...
    for i, row in data.iterrows():
        ...
        if row["unip_id"] not in already_searched_ids:
            ...
            with open(f"{temp_path}tmp.fasta", 'w') as tmp_file:
                ...
                already_searched_ids.append(row["unip_id"])
                ...
                if best_result_a:
                    ...
                    if '_' in best_result_a[0]:
                        data.loc[i, "pdb_id"] = best_result_a[0].split('_')[0]
                        data.loc[i, "chain"] = best_result_a[0].split('_')[1]
                    ...
                    # later)
                        ...
                        data.loc[i, "pdb_id"] = best_result_a[0][:4]
                        data.loc[i, "chain"] = '-'
                        already_searched_res.append((best_result_a[0][:4], '-', ' '.join(best_result_a)))

                    data.loc[i, "all_results"] = ' '.join(best_result_a)
                ...
                # furthermore save entry index to not_found (will be retrieved from alphafold, if possible)
                    ...
                    data.loc[i, "pdb_id"] = None
                    data.loc[i, "chain"] = None
                    already_searched_res.append((None, None, ''))
                    not_found.append(i)

        # If reencountered an uniprot id, copy result from already_searched_res
        else:
            id_a, chain_a, best_result_a = already_searched_res[already_searched_ids.index(row["unip_id"])]
            data.loc[i, "pdb_id"] = id_a
            data.loc[i, "chain"] = chain_a
            data.loc[i, "all_results"] = best_result_a
    ...
    not_found_proteins = data.iloc[not_found].unip_id.unique()
